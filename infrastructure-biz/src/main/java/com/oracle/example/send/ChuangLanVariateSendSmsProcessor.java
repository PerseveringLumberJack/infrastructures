...
@Slf4j
@Component
public class ChuangLanVariateSendSmsProcessor extends AbstractSendSmsProcessor<ChuangLanDto.VariateRequest, ChuangLanDto.VariateResponse, ChuangLanErrorEnum> {
    ...
    @Override
    protected UpdateRecordInfo queryUpdateRecordInfo(ChuangLanDto.VariateResponse r) {

        return new UpdateRecordInfo<>(r.getError(),r.getCode(),
                ChuangLanErrorEnum.class,ChuangLanErrorEnum.SUCCESS);
    }

    @Override
    protected ChuangLanDto.VariateResponse toResponseObject(String result) {

        if(StringUtils.isNotEmpty(result)){
            JSONObject jsonObject = JSON.parseObject(result);
            return jsonObject.toJavaObject(ChuangLanDto.VariateResponse.class);
        }
        return null;
    }
    ...
    protected ChuangLanDto.VariateRequest toRequestObject(ChannelEntity channel, SmsSendRecordEntity record) {
        ...
        return new ChuangLanDto.VariateRequest(configStore.CHUANGLANURLPRE()+configStore.CHUANGLANSENDSMSURL(),record.getRequestBody());
    }
